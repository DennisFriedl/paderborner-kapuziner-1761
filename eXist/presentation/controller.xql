...
if ($exist:path eq '') then
    ...
    if ($exist:path eq "/") then
        ...
        if (ends-with($exist:resource, ".html")) then
            ...
            (: hier übergebe ich auch exist-resource als Attribute, so dass view.xql darauf zugreiffen kann :)
            <dispatch
                xmlns="http://exist.sourceforge.net/NS/exist">
                <view>
                    <forward
                        url="{$exist:controller}/modules/view.xql">
                        <set-attribute name="exist-resource" value="{$exist:resource}"></set-attribute>
                        </forward>
                </view>
                <error-handler>
                    <forward
            ...
            if (contains($exist:path, "/$shared/")) then
                ...
                if (contains($exist:path, "$app-root")) then
                    <dispatch
                        xmlns="http://exist.sourceforge.net/NS/exist">
                        <forward
                            url="{$exist:controller}/{substring-after($exist:path, '$app-root/')}">
                            <set-header
                                name="Cache-Control"
                                value="no"/>
                        </forward>
